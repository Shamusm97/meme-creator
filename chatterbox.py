...
class ChatterboxTTSRequest:
    ...
    def __post_init__(self):
        ...
        if self.voice_mode == "predefined" and not self.predefined_voice_id:
            raise ValueError("predefined_voice_id is required when voice_mode is 'predefined'")
        
        if self.voice_mode == "clone" and not self.reference_audio_filename:
            raise ValueError("reference_audio_filename is required when voice_mode is 'clone'")
...
class ChatterboxTTSClient:
    ...
    def save_stream_to_file(self, response: requests.Response, output_path: Union[str, Path]) -> dict:
        """
    ...
    def synthesize_to_file(self, request: ChatterboxTTSRequest, output_path: Union[str, Path]) -> dict:
        """
    ...
    def synthesize_batch(self, texts: list[str], output_dir: Union[str, Path], 
                        ...
                        filename_prefix: str = "speech") -> list[dict]:
        """
    ...
    def get_reference_files(self) -> dict:
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/get_reference_files",
                timeout=self.timeout
            )
            response.raise_for_status()
            return {
    ...
    def get_predefined_voices(self) -> dict:
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/get_predefined_voices",
                timeout=self.timeout
            )
            response.raise_for_status()
            return {
    ...
    def upload_reference_audio(self, file_path: Union[str, Path], 
        ...
        if not force_overwrite:
            ...
            if existing_files['success']:
                existing_names = [f.get('name', f) if isinstance(f, dict) else f 
    ...
    def upload_predefined_voice(self, file_path: Union[str, Path],
                               force_overwrite: bool = False) -> dict:
        """
        ...
        if not force_overwrite:
            ...
            if existing_voices['success']:
                existing_names = [v.get('name', v) if isinstance(v, dict) else v 
    ...
    def batch_upload_reference_files(self, file_paths: list[Union[str, Path]],
                                   force_overwrite: bool = False) -> list[dict]:
        """
        ...
        for file_path in file_paths:
            ...
            print(f"{status} {filename}: {result.get('message', result.get('error', 'Unknown'))}")
    ...
    def list_available_voices(self) -> dict:
        ...
        return {
            ...
            'total_voices': (len(predefined.get('voices', [])) + 
                           len(reference.get('files', []))),
            'errors': {
...
class TikTokTTSManager:
    ...
    def generate_speech_files(self, script: str, output_dir: Union[str, Path],
                            voice_mapping: dict[str, str], 
                            base_config: ChatterboxTTSRequest) -> tuple[list[dict], list[dict]]:
        """
        ...
        for i, (character, dialogue) in enumerate(script_lines):
            ...
            request = ChatterboxTTSRequest(
                text=dialogue,
                voice_mode=base_config.voice_mode,
                predefined_voice_id=voice_id if base_config.voice_mode == "predefined" else None,
                reference_audio_filename=voice_id if base_config.voice_mode == "clone" else None,
                output_format=base_config.output_format,
                split_text=base_config.split_text,
                chunk_size=base_config.chunk_size,
                voice_profile=base_config.voice_profile
            ...
            # Generate filename
            filename = f"{i:03d}_{character.lower().replace(' ', '_')}.{request.output_format}"
            output_path = output_dir / filename
